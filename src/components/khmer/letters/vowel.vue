<template>
  ...
  <!-- Top action panel of crud -->
    <div class="flex title-bar border-b border-gray-200">
      ...
      <div class="flex w-64 h-10 py-1 title " >
        <Icon size="25" class="vcb-icon" >
          <svg width="29.299" height="73.245" viewBox="0 0 29.299 73.245" xmlns="http://www.w3.org/2000/svg"><g id="svgGroup" stroke-linecap="round" fill-rule="evenodd" font-size="9pt" stroke="#000" stroke-width="0.25mm" fill="#000" style="stroke:#000;stroke-width:0.25mm;fill:#000"><path d="M 0 7.327 L 0 0.003 L 9.766 0.003 Q 25.264 0.003 28.464 12.301 A 28.775 28.775 0 0 1 29.297 19.534 L 29.297 73.245 L 20.996 73.245 L 20.996 19.534 Q 20.996 9.164 12.891 7.604 A 16.549 16.549 0 0 0 9.766 7.327 L 0 7.327 Z" vector-effect="non-scaling-stroke"/></g></svg>
        </Icon>
    ...
    <div class="vcb-table-panel relative w-full">
      <!-- <div class="" v-for="(invoice,index) in invoices" :key="index" >{{  invoice.id  }}</div> -->
      ...
      <!-- <div class="flex justify-center w-full  p-4 text-green-700 font-ktr pt-16" style="font-size: 100pt; " >
        <span class="border-4 p-4 w-60 h-60 rounded m-8 text-blue-500" >{{ visibleConsonant }}</span>
        <span class="border-4 p-4 m-8 " >+</span> 
        <span class="border-4 p-4 m-8 text-green-500" >{{ visibleVowel }}</span>
        <span class="border-4 p-4 m-8" >=</span>
        <span class="border-4 p-4 m-8 text-yellow-500" >{{ visibleConsonant+visibleVowel }}</span>
      </div> -->
      
        <div class="flex flex-wrap justify-center justify-start text-green-700 font-ktr px-4 py-8 " >
          <div v-for="(v,index) in vowels" :key="index" class="relative vowel m-4 text-8xl border p-12 bg-white rounded text-blue-500 cursor-pointer hover:shadow-lg hover:border-green-300" >{{ 'ក'+v }}
...
export default {
  ...
  setup(){
    ...
    // setInterval( updateConsonantAndVowel , 6000 )
